...
pub struct AiUiStateBuilder {
    ...
    library_prompt_path: Option<std::path::PathBuf>,
...
impl AiUiStateBuilder {
    ...
    /// Splice OpenUI's pre-rendered library prompt verbatim. When set, this
    /// replaces the base preamble + per-component rendering in
    /// [`ai_ui_prompt::PromptBuilder`].
    pub fn library_prompt(mut self, text: impl Into<String>) -> Self {
        self.library_prompt = Some(text.into());
        self
    }

    /// Same as [`library_prompt`], but reads from a path generated by the
    /// `generate-ai-ui-prompt` CLI shipped with `@nube/ai-ui-react`.
    pub fn library_prompt_file<P: Into<std::path::PathBuf>>(mut self, path: P) -> Self {
        self.library_prompt_path = Some(path.into());
        self
    }
    ...
    pub fn build(self) -> Result<AiUiState, BuildError> {
        ...
        let library_prompt = match (self.library_prompt, self.library_prompt_path) {
            (Some(t), _) => Some(t),
            (None, Some(p)) => Some(std::fs::read_to_string(&p).map_err(|source| {
                ai_ui_prompt::PromptError::Io {
        ...
        if let Some(lib) = library_prompt {
            prompt = prompt.library_prompt(lib);
        }
